...
use super::{HmrEvent, WsMessage, WsStream};
    ...
    pub async fn handle(
        ...
        match self {
            ...
            Self::Update { updates } => {
                ...
                if !paths.is_empty() {
                    let _ = event_tx.send(HmrEvent::Update { paths });
                }
            }
            Self::Prune { paths } => {
                for path in &paths {
                ...
                let _ = event_tx.send(HmrEvent::Prune { paths });
            }
            Self::FullReload {
                path: _,
                triggered_by,
                ...
                let _ = event_tx.send(HmrEvent::FullReload);
            }
