...
pub fn sanitize_path(dir: impl AsRef<Path>) -> Result<PathBuf, FsError> {
    let dir = dir.as_ref();
    let dir = fs::canonicalize(dir).map_err(|e| FsError::sanitize(e, dir))?;
    let dir = dunce::simplified(&dir).to_owned();
    Ok(dir)
}

fn ancestor_with_target(dir: impl AsRef<Path>, target: Target) -> Result<PathBuf, FsError> {
    let dir = sanitize_path(dir)?;
    GetDir {
        dir: dir.clone(),
        targets: vec![target],
        ..Default::default()
...
pub fn ancestor_with_file(
    dir: impl AsRef<Path>,
    filename: impl AsRef<str>,
) -> Result<PathBuf, FsError> {
    let target = Target::File(FileTarget::new(filename.as_ref()));
    ancestor_with_target(dir, target)
}

pub fn ancestor_with_subdir(
    dir: impl AsRef<Path>,
...
pub fn cargo_workspace() -> Result<PathBuf, FsError> {
    let dir = std::env::current_dir().map_err(FsError::GetCwd)?;
    ancestor_with_file(&dir, "Cargo.lock")
}
...
pub fn pnpm_workspace() -> Result<PathBuf, FsError> {
    let dir = std::env::current_dir().map_err(FsError::GetCwd)?;
    ancestor_with_file(&dir, "pnpm-workspace.yaml")
}

pub fn chipbox_solid_render() -> Result<PathBuf, FsError> {
    Ok(pnpm_workspace()?.join("node/chipbox-solid-render"))
}

pub fn ts_bindings_output() -> Result<PathBuf, FsError> {
    Ok(chipbox_solid_render()?.join("generated"))
}

pub fn portaudio() -> Result<PathBuf, FsError> {
    Ok(cargo_workspace()?.join("portaudio"))
}

/// Opens a file for appending and clears its contents beforehand.
pub fn open_clear_file(path: impl AsRef<Path>) -> Result<fs::File, FsError> {
    let path = path.as_ref();
    // Create the directory.
    fs::create_dir_all(path.parent().expect("parent dir exists")).map_err(FsError::CreateDir)?;
    // Create the file.
    fs::File::options()
        .write(true)
        .truncate(true)
...
pub fn cargo_target_subdir(subdir: impl std::fmt::Display) -> Result<PathBuf, FsError> {
    let path = crate::fs::cargo_workspace()?.join(format!("target/{subdir}"));
    Ok(path)
}

/// Returns a path to the output directory for a given cargo profile.
pub fn cargo_output_dir(profile: CargoProfile) -> Result<PathBuf, FsError> {
    let path = crate::fs::cargo_target_subdir(profile.target_folder())?;
    Ok(path)
}
...
pub fn built_chipbox_binary(profile: CargoProfile) -> Result<PathBuf, FsError> {
    let path = crate::fs::cargo_workspace()?.join(format!(
        "target/{}/{}",
